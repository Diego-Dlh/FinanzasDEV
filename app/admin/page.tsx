  ...
  return (
    <div className="min-h-screen bg-[#0f1117] text-white">
      ...
      <header className="border-b border-white/10 px-6 py-4 flex items-center justify-between">
        <div className="flex items-center gap-3">
          ...
          <div>
            ...
            <p className="text-[10px] uppercase tracking-[0.3em] text-white/40">Lumina Finance</p>
          </div>
        </div>
        <button
          onClick={logout}
          className="flex items-center gap-2 rounded-xl px-3 py-2 text-sm text-white/60 hover:text-white hover:bg-white/10 transition"
        >
          <LogOut size={15} />
          <span>Salir</span>
        </button>
      ...
      <main className="max-w-5xl mx-auto px-6 py-8 space-y-8">
        ...
        {error && (
          <div className="rounded-2xl bg-red-900/40 border border-red-500/30 px-4 py-3 text-sm text-red-300">
            {error}
        ...
        {/* ── Registration Key ────────────────────────────────────────────── */}
        <section className="rounded-[24px] bg-white/5 border border-white/10 p-6 space-y-5">
          <div className="flex items-center gap-3">
          ...
          {activeKey ? (
            ...
            <p className="text-sm text-white/40 italic">No hay clave activa.</p>
          )}
        ...
        {/* ── Users table ─────────────────────────────────────────────────── */}
        <section className="rounded-[24px] bg-white/5 border border-white/10 p-6 space-y-4">
          <div className="flex items-center gap-3">
            <Users size={18} className="text-secondary" />
            <h2 className="font-semibold text-base">Usuarios registrados</h2>
            <span className="ml-auto text-xs text-white/40 bg-white/10 px-2 py-1 rounded-full">
              {users.length} total
            </span>
          </div>
            ...
            <table className="w-full text-sm">
              <thead>
                <tr className="border-b border-white/10">
                  <th className="text-left py-3 px-2 text-xs uppercase tracking-wider text-white/40 font-medium">#</th>
                  <th className="text-left py-3 px-2 text-xs uppercase tracking-wider text-white/40 font-medium">Nombre</th>
                  <th className="text-left py-3 px-2 text-xs uppercase tracking-wider text-white/40 font-medium">Email</th>
                  <th className="text-left py-3 px-2 text-xs uppercase tracking-wider text-white/40 font-medium">Actividad</th>
                  <th className="text-left py-3 px-2 text-xs uppercase tracking-wider text-white/40 font-medium">Registro</th>
              ...
              <tbody className="divide-y divide-white/5">
                {users.map((u, i) => (
                  <tr key={u.id} className="hover:bg-white/5 transition group">
                    <td className="py-3.5 px-2 text-white/30 text-xs">{i + 1}</td>
                    ...
                    <td className="py-3.5 px-2 text-white/60 text-xs">{u.email}</td>
                    <td className="py-3.5 px-2">
                      <div className="flex gap-3 text-xs text-white/40">
                        <span className="flex items-center gap-1">
                          <span className="text-emerald-400">↑</span>{u._count.incomes}
                        </span>
                        <span className="flex items-center gap-1">
                          <span className="text-red-400">↓</span>{u._count.expenses}
                        </span>
                      </div>
                    </td>
                    <td className="py-3.5 px-2 text-white/40 text-xs whitespace-nowrap">{fmtDate(u.createdAt)}</td>
                    <td className="py-3.5 px-2">
                      {!u.isAdmin && (
                        <button
                          onClick={() => deleteUser(u.id, u.name)}
                          className="opacity-0 group-hover:opacity-100 transition w-8 h-8 flex items-center justify-center rounded-xl hover:bg-red-900/40 text-red-400"
